...
import (
	...
	"github.com/carsenk/dnrd/chaincfg/chainhash"
	"github.com/carsenk/dnrd/wire"
...
func (cs *CoinSet) TotalValue() (value dnrutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue dnrutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue dnrutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount dnrutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue dnrutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue dnrutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue dnrutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue dnrutil.Amount, coins []Coin) (Coins, error) {
...
// dnrutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() dnrutil.Amount {
	return dnrutil.Amount(c.txOut().Value)
}
