...
import (
	...
	"github.com/carsenk/dnrutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := dnrutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(dnrutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := dnrutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := dnrutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := dnrutil.NewBlockFromBytes(blockBytes)
	if err != nil {
